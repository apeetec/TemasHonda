<?php
/**
 * Template de Taxonomia & Galeria Oficial de Sorteios (SIPAT Honda)
 * 
 * Exibe os brindes sorteados e os colaboradores contemplados com ênfase no Grupo e Unidade
 */

get_header();

$current_term = get_queried_object();
$initial_filter = (!empty($current_term) && isset($current_term->slug)) ? $current_term->slug : 'todos';
$current_name   = (!empty($current_term) && isset($current_term->name)) ? $current_term->name : 'Todos os Sorteios';

// Busca todas as datas e sorteios cadastrados
$all_dates = sipat_get_all_event_dates();
$all_sorteios = sipat_get_all_sorteios_list(); // Carrega todos para permitir filtro instantâneo via JS
$total_sorteios = count($all_sorteios);

// Calcula total de ganhadores
$total_ganhadores = 0;
foreach ($all_sorteios as $s) {
...
<div class="sipat-winners-page">
    ...
    <section class="winners-hero">
        <div class="container hero-container">
            ...
            <p class="hero-subtitle">
                Parabéns a todos os colaboradores que gabaritaram os questionários diários e foram contemplados nos sorteios por Grupo e Unidade!
            </p>
            ...
            <div class="hero-stats">
                ...
                <div class="stat-box highlight">
                    ...
                    <span class="stat-label"><i class="fa-solid fa-users"></i> Ganhadores Premiados</span>
                </div>
                <div class="stat-box">
    ...
    <div class="container winners-content">
        ...
        <!-- Barra de Filtros Interativa (Sem Recarregamento de Página) -->
        <div class="filters-container">
            <div class="filters-title">
                <i class="fa-solid fa-filter"></i> Filtrar por Dia da SIPAT:
            </div>
            <div class="filters-buttons" id="filtersNav">
                <!-- Botão Todos os Sorteios -->
                <button type="button" class="filter-pill <?php echo ($initial_filter === 'todos') ? 'active' : ''; ?>" data-filter="todos">
                    <i class="fa-solid fa-border-all"></i> Todos os Sorteios (<?php echo $total_sorteios; ?>)
                </button>

                <!-- Botão Semana Completa -->
                <button type="button" class="filter-pill special <?php echo ($initial_filter === 'todas') ? 'active' : ''; ?>" data-filter="todas">
                    🌟 Semana Completa
                </button>

                <!-- Dias Individuais -->
                <?php foreach ($all_dates as $d): ?>
                    <button type="button" class="filter-pill <?php echo ($initial_filter === $d['slug']) ? 'active' : ''; ?>" data-filter="<?php echo esc_attr($d['slug']); ?>">
        ...
        <?php if (empty($all_sorteios)): ?>
            <div class="empty-state-card">
                <div class="empty-icon"><i class="fa-solid fa-hourglass-half"></i></div>
                <h2>Nenhum sorteio divulgado ainda!</h2>
                <p>Os sorteios são realizados após as palestras com os colaboradores que acertaram 100% das questões. Fique ligado!</p>
            </div>
        <?php else: ?>
            <div class="prizes-grid" id="prizesGrid">
                <?php foreach ($all_sorteios as $sorteio): 
            ...
            <!-- Estado de busca vazia no filtro -->
...
<style>
/* ==========================================================================
   ESTILOS PREMIUM - GALERIA DE SORTEIOS SIPAT COM ÊNFASE NO GRUPO
   ========================================================================== */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Se houver um filtro inicial diferente de 'todos'
    const initialFilter = '<?php echo esc_js($initial_filter); ?>';
    if (initialFilter && initialFilter !== 'todos') {

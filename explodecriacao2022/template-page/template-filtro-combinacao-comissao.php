...
/* Template Name: Filtro Comissão */

get_header();

$grupo_principal = get_user_meta(get_current_user_id(), 'user_field_comissao_grupo', true);
  ...
  <div class="content">
    ...
    if($_POST) {
      ...
      foreach($ids_update as $post) {
        ...
        $votos_comissao = get_post_meta($post,'desenhos_box_votos_comissao',true);

        // ATUALIZA VOTOS NORMAIS
        ...
        // ATUALIZA VOTOS DE COMISSAO
        if(empty($votos_comissao) || $votos_comissao == '') {
          update_post_meta($post,'desenhos_box_votos_comissao',1);
        }
        else {
          $votos_comissao = $votos_comissao + 1;
          update_post_meta($post,'desenhos_box_votos_comissao',$votos_comissao);
        }
  ...
  <div class="filtros">
    ...
    <div class="dependentes">
      <div class="container">
        <h1 class="center">
          <?php
          if($grupo_principal == 'grupo-1') {
            echo 'Grupo 1';
          }
          if($grupo_principal == 'grupo-2') {
            echo 'Grupo 2';
          }
          if($grupo_principal == 'grupo-3') {
            echo 'Grupo 3';
          }
          ?>
        </h1>
...
$('#left form input').on('change',function(){
    ...
    $.each(getArrVal_categorias, function(index, item_cat) {
        ...
        if(!$('.loop-drawings .single').hasClass(item_cat)) {
          // $('.loop-drawings .single').hide();
        }
        else {
          // $('.' + item_cat + '.' + item_grupo).show();
          drawings_to_show.push('.' + item_cat);
        }

// Função de colar os nomes
$(document).ready(function() {
    $('#campos_nomes').on('paste', function(event) {
      ...
      rows.forEach(function(row) {
        ...
        cells.forEach(function(cell) {
          var input = $('<input type="text" class="' + cell + '" value="' + cell + '" name="nome[]">');
      ...
      $('#exibir_nomes').html(table);
      console.log(rows.length + ' linhas adicionadas');
    });
    $('#exibir_nomes').on('click', '.excluir', function() {
      $(this).parent().find('input, span').remove();
    });
  });